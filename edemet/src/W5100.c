...
uint8_t large_rx_buffer[EM_VECTOR_SIZE + 1024];
...
int W5100_Init(W5100_Handle *handle, W5100_Config *config) {
  ...
  XSpiPs_Config *spi_config =
      XSpiPs_LookupConfig(config->spi_instance->Config.BaseAddress);
  if (spi_config == NULL) {
    return XST_FAILURE;
  }
  ...
  status = XSpiPs_SetOptions(config->spi_instance,
                             XSPIPS_MASTER_OPTION | XSPIPS_MANUAL_START_OPTION);
  if (status != XST_SUCCESS) {
...
uint8_t W5100_ReadByte(W5100_Handle *handle, uint16_t addr) {
  ...
  uint8_t tx[4] = {W5100_SPI_OP_READ, (addr >> 8) & 0xFF, addr & 0xFF, 0};
  uint8_t rx[4];
...
void W5100_WriteByte(W5100_Handle *handle, uint16_t addr, uint8_t data) {
  ...
  uint8_t tx[4] = {W5100_SPI_OP_WRITE, (addr >> 8) & 0xFF, addr & 0xFF, data};
  uint8_t rx[4];
  W5100_SetSS(handle);
  XSpiPs_PolledTransfer(handle->config.spi_instance, tx, rx, 4);
  W5100_ResetSS(handle);
 ...
 */
void W5100_ReadBuffer(W5100_Handle *handle, uint16_t addr, uint8_t *buf,
                      uint16_t len) {
  /*W5100_SPIBegin(handle);
  W5100_SetSS(handle);
 ...
 */
void W5100_WriteBuffer(W5100_Handle *handle, uint16_t addr, const uint8_t *buf,
                       uint16_t len) {
  /*
  W5100_SPIBegin(handle);
  W5100_SetSS(handle);
...
void W5100_WriteWord(W5100_Handle *handle, uint16_t addr, uint16_t data) {
  uint8_t tx[5] = {W5100_SPI_OP_WRITE, (addr >> 8) & 0xFF, addr & 0xFF,
 ...
 */
void W5100_SendData(W5100_Handle *handle, uint8_t sock, const uint8_t *buf,
                    uint16_t len) {
  uint16_t base = W5100_S0_BASE + (sock * 0x0100);
 ...
 */
void W5100_RecvData(W5100_Handle *handle, uint8_t sock, uint8_t *buf,
                    uint16_t len) {
  uint16_t base = W5100_S0_BASE + (sock * 0x0100);
 ...
 *
 * Packing format: 8 values (v0..v7) of 13 bits each packed into 13 bytes
 * LSB-first
 */
static void unpack_13bit_values(const uint8_t *packed, uint16_t *unpacked,
                                uint32_t num_values) {
...
static void TCP_Server_Task(void *pvParameters) {
  ...
  W5100_WriteByte(&w5100_handle, base + W5100_Sn_MR,
                  W5100_SnMR_TCP | W5100_SnMR_ND);
  ...
  while (1) {
    ...
    case W5100_SNSR_ESTABLISHED: {
      // Print needed allow the firmware to accept new connections after
      // disconnect xil_printf("TCP Server: W5100_SNSR_ESTABLISHED\r\n");
      ...
      if (recv_size > 0) {
        ...
        case CMD_CODE_CONFIG:
          ...
          xSemaphoreGive(op->mutex);
          xil_printf(
              "operationTime: %d, delayTime: %d, signalSamplePeriodMs: %d\r\n",
        ...
        case CMD_CODE_CONFIG_READ:
          ...
          response[3] = ((op->operationTime) >> 24) & 0xFF;
          response[4] = ((op->operationTime) >> 16) & 0xFF;
          response[5] = ((op->operationTime) >> 8) & 0xFF;
          response[6] = (op->operationTime) & 0xFF;

          // Delay de reproduccion
          response[7] = ((op->delayTime) >> 24) & 0xFF;
          response[8] = ((op->delayTime) >> 16) & 0xFF;
          response[9] = ((op->delayTime) >> 8) & 0xFF;
          response[10] = (op->delayTime) & 0xFF;

          // Periodo de muestreo de la señal
          response[11] = ((op->signalSamplePeriodMs) >> 24) & 0xFF;
          response[12] = ((op->signalSamplePeriodMs) >> 16) & 0xFF;
          response[13] = ((op->signalSamplePeriodMs) >> 8) & 0xFF;
          response[14] = (op->signalSamplePeriodMs) & 0xFF;
          response_length = 15;
          break;
        ...
        case CMD_CODE_DISCOVER:
          ...
          response[3] = (device_internal_id >> 24) & 0xFF;
          response[4] = (device_internal_id >> 16) & 0xFF;
          response[5] = (device_internal_id >> 8) & 0xFF;
          response[6] = device_internal_id & 0xFF;
          response[7] = VERSION_MAYOR;
          response[8] = VERSION_MINOR;
        ...
        case CMD_CODE_GET_EM_CONFIG:
          ...
          em_id = recv_buf[3];
          if (em_ring_index > 2) {
            response[2] = RESPONSE_ERROR_INVALID_RING;
            response_length = 3;
            break;
          } else if (em_id > 9) {
            response[2] = RESPONSE_ERROR_INVALID_EM;
            response_length = 3;
            break;
          } else {
            response[2] = RESPONSE_SUCCESS;
          }
          em_array_idx = em_ring_index * 10 + em_id;

          response[3] = em_ring_index;
          response[4] = em_id;
          response[5] = op->em[em_array_idx].mode;
          response[6] = op->em[em_array_idx].type;
          response[7] = op->em[em_array_idx].duty_cycle;
          response[8] = op->em[em_array_idx].frequency_factor;
          response[9] =
              ((op->em[em_array_idx].magnetic_field_offset) & 0xFF00) >> 8;
          response[10] = (op->em[em_array_idx].magnetic_field_offset) & 0x00FF;
          response[11] =
              ((op->em[em_array_idx].magnetic_field_amplitude) & 0xFF00) >> 8;
          response[12] =
              (op->em[em_array_idx].magnetic_field_amplitude) & 0x00FF;
        ...
        case CMD_CODE_WRAPPER:
          tmp_wrapper_pkt_index =
              ((uint32_t)recv_buf[3] << 24) | ((uint32_t)recv_buf[4] << 16) |
              ((uint32_t)recv_buf[5] << 8) | (uint32_t)recv_buf[6];
          ...
          if ((uint64_t)tmp_wrapper_pkt_index +
                  (uint64_t)tmp_wrapper_pkt_length >
              sizeof(large_rx_buffer)) {
            xil_printf("ERROR: Buffer overflow! Index: %u, Length: %u, Buffer "
                       "size: %u\r\n",
                       tmp_wrapper_pkt_index, tmp_wrapper_pkt_length,
                       (uint32_t)sizeof(large_rx_buffer));
          ...
          memcpy(&large_rx_buffer[tmp_wrapper_pkt_index],
                 &recv_buf[WRAPPER_HEADER_SIZE], tmp_wrapper_pkt_length);
      ...
      if (wrapped_cmd_available) {
        ...
        case CMD_CODE_VECTOR_HEADER:
          ...
          response[0] = (cmd & 0xFF00) >> 8;
          response[1] = (cmd & 0x00FF);
          response[2] = process_vector_header_cmd(
              op, large_rx_buffer, wrapper_pkt_index + wrapper_pkt_length);
          // response_length = 3;
          response_length = 0;
          break;
        ...
        default:
          ...
          response[0] = (cmd & 0xFF00) >> 8;
          response[1] = (cmd & 0x00FF);
          response[2] = RESPONSE_ERROR_INVALID_MODE;
          response_length = 3;
          break;
        }
    ...
    case W5100_SNSR_CLOSE_WAIT:
      ...
      while (W5100_ReadByte(&w5100_handle, base + W5100_Sn_SR) !=
             W5100_SNSR_CLOSED) {
        vTaskDelay(pdMS_TO_TICKS(1));
      }
...
void expand_looped_13bit_samples(uint8_t *buf, uint32_t original_samples,
...
void generateWrappedDataSend(uint32_t start_index) {
  // This function can be used to prepare a response that needs to be sent in
  // multiple packets For example, if you want to send a large status report or
  // a large vector back to the client You would set up the large_rx_buffer with
  // the data to send, and then implement logic in the TCP_Server_Task to send
  // it in chunks with the appropriate wrapper header (CMD_CODE_WRAPPER)
  // indicating START, CONTINUE, END packets
